...
const SendEmails = () => {
      ...
      const response = await fetch('https://rexmailerserver.vercel.app/send-email', {
        ...
        body: JSON.stringify({
          template: {
            ...
            content: addPdfLinksToContent(selectedTemplate), // Function to add PDF links if exists
          },
  ...
  // Function to add PDF links to content if attachments exist
  const addPdfLinksToContent = (template) => {
    let contentWithLinks = template.htmlContent;

    if (template.attachments && template.attachments.length > 0) {
      template.attachments.forEach(attachment => {
        if (attachment.type === 'pdf') {
          // Construct the link HTML and replace placeholder in content
          const pdfLinkHTML = `<a href="${attachment.url}" target="_blank">${attachment.filename}</a>`;
          contentWithLinks = contentWithLinks.replace(`{{pdfLink_${attachment.filename}}}`, pdfLinkHTML);
        }
      });
    }
